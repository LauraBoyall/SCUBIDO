...
# Show this line to users, indicating normal function usage
reconstruction <- SCUBIDO_reconstruct(calibration_data = calibrated_data, plot_graph = TRUE)

# Suppress unwanted console output (including print, messages, and progress bars)
temp <- tempfile()

# Redirect output to a temporary file to suppress all messages
sink(temp)
capture.output({
  reconstruction <- SCUBIDO_reconstruct(calibration_data = calibrated_data, plot_graph = TRUE)
}, file = temp)
